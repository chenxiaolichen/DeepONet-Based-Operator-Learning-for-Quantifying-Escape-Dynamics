...
data_dir = r"E:\gene\PFS\sig\reflect"
...
def solve_pde(b_fn, sigma_fn, x):
    ...
    i = 0
    A[i, i] = -3 * Chh[i] - 3 * b_vals[i] / (2 * h)
    A[i, i+1] = 4 * Chh[i] + 4 * b_vals[i] / (2 * h)
    A[i, i+2] = -1 * Chh[i] - 1 * b_vals[i] / (2 * h) if N >= 3 else 0.0
    rhs[i] = 0.0
...
def generate_batch_data(N, P, n_cheb, coeff_range, x_grid=x_base):
    pbar = trange(N, desc=f"generate data")
...
def load_train_test_data():
    ...
    assert np.allclose(data['x_sensor'], x_sensor), "sensor grid not match"
    return (
...
for i, (x_vals, s_true, s_pred, sigma, l2_err) in enumerate(fixed_sigma_results):
    ...
    plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(data_dir, 'pre_different σ.png'), dpi=300)
plt.show()
...
plt.savefig(os.path.join(data_dir, 'loss curve.png'), dpi=300)
plt.show()