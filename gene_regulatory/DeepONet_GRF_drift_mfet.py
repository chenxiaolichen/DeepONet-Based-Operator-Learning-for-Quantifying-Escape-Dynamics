...
data_dir = r"G:\代码_new\gene\GRF\drift\met\test6"
...
def save_training_data(f_train, y_train, s_train):

    save_path = os.path.join(data_dir, "training_data.npz")
    np.savez_compressed(
        save_path,
        f_train=f_train,
        y_train=y_train,
...
def load_training_data():
    ...
    load_path = os.path.join(data_dir, "training_data.npz")
...
class DataGenerator(data.Dataset):
    def __init__(self, f_sensor, y, s, batch_size=64):
...
def generate_training_data(N, P):
    ...
    for _ in pbar:
        f, y, s = generate_one_training_data(P)
        f_list.append(f)
        y_list.append(y)
        s_list.append(s)
    ...
    s_train = np.concatenate(s_list, axis=0)
    return f_train, y_train, s_train
...
class DeepONet(nn.Module):
    ...
    def train_model(self, dataset, epochs):
        data_loader = data.DataLoader(dataset, batch_size=dataset.batch_size, shuffle=True)
        pbar = trange(epochs)
        for epoch in pbar:
            total_loss = 0.0
            for (f_batch, x_batch), s_batch in data_loader:
                # 移动数据到设备
...
if __name__ == '__main__':
    ...
    if os.path.exists(training_data_path):
        f_train, y_train, s_train = load_training_data()
    else:
        f_train, y_train, s_train = generate_training_data(N_train, P_per_group)
        save_training_data(f_train, y_train, s_train)
    dataset = DataGenerator(f_train, y_train, s_train, batch_size=batch_size)


    branch_layers = [m, 128, 128, 128, 128]
    trunk_layers = [1, 128, 128, 128, 128]
    model = DeepONet(branch_layers, trunk_layers).to(device)

    model.train_model(dataset, epochs=30000)
